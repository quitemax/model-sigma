...
\]
where $D$ is a distance measure (e.g.\ earth mover's distance) between the system's actual transition distribution and the distribution after cutting it according to partition $P$. A recurring criticism (echoing Aaronson's informal objection) is that $\Phi$ collapses a rich causal structure into one number, with no distinction of \emph{kind} or \emph{location} of integration. This is conceptually related to, but distinct from, Wolfram's computational irreducibility --- the idea that some computations admit no shortcut and must be run step by step to know their outcome.
...
\textbf{Nested Observer Windows} \cite{fink2024} independently propose computing $\Phi$ separately at nested hierarchical levels and examining the ratio between levels as a test between panpsychist and emergentist readings of consciousness --- close in spirit to our layered profile $\Phi(s,\ell,t)$, though without functional weighting or dynamics.
...
\textbf{Predictive processing and active inference} \cite{friston2010,deane2021} motivate the self-prediction-error formalism; the ongoing adversarial collaboration between IIT and predictive-processing camps (INTREPID, Templeton World Charity Foundation) confirms that bridging the two remains an open, active research question.
...
No prior work known to us combines hierarchical $\Phi$, functional weighting, self-prediction-error-driven dynamics, and soft exclusion into one formalism with proven boundary properties; the synthesis appears original, though every component has independent precedent.

\section{The $\Sigma$ Formalism}

\subsection{Hierarchical profile and functional weight}
A system $S$ is divided into functional layers $\ell=1,\dots,L$ (e.g.\ homeostasis $\to$ perception/motor $\to$ world-model $\to$ self-model). Instead of one scalar, we obtain a profile $\Phi(s)=(\Phi_1(s),\dots,\Phi_L(s))$, collapsed via a functional weight:
\[
\Sigma(t) = \int_0^L w(\ell,t)\,\Phi(s,\ell,t)\,d\ell \qquad \text{(continuum idealization; discrete sum in practice, \S\ref{sec:pathology})}.
\]
Inter-layer terms $\Phi_{\ell,\ell-1}$ (structural coupling between adjacent layers, computed via the same distance measure as base $\Phi$) are non-negative by construction, being themselves a distance.
...
Two designs for the weight were considered: (1) $w(\ell,t)=w_0(\ell)+\beta g_\ell(t)e_\ell(t)$, in which $\Sigma$ and a self-correction measure share a term and are therefore tautologically correlated by construction; (2) $w(\ell,t)=w_0(\ell)$, static, under which
\[
\Sigma(t) = \int_0^L w_0(\ell)\,\Phi(s,\ell,t)\,d\ell, \qquad \Gamma(t) = \int_0^L g_\ell(t)e_\ell(t)\,d\ell
\]
are independent by construction, so any measured correlation between them in a real system is an empirical finding, not a definitional artifact. We adopt (2) as canonical.
...
\end{theorem}
\begin{proof}
$\Phi(s,\ell,t)\ge 0$ (a distance) and $w_0(\ell)\ge 0$ by A1; the integral of a product of non-negative functions is non-negative.
\end{proof}
...
\end{theorem}
\begin{proof}
Let $f(\ell)=\Phi(s,\ell,t)-\bar\Phi(t)$, $h(\ell)=g_\ell(t)e_\ell(t)-\overline{ge}(t)$. For any $\lambda$, $\frac1L\int(\sigma_{ge}f-\sigma_\Phi h)^2 \ge 0$ expands to $\sigma_{ge}^2\sigma_\Phi^2 + \sigma_\Phi^2\sigma_{ge}^2 \ge 2\sigma_\Phi\sigma_{ge}\,\mathrm{Cov}_\ell$, giving $\mathrm{Cov}_\ell\le\sigma_\Phi\sigma_{ge}$, i.e.\ $A\le1$; the symmetric argument with $(\sigma_{ge}f+\sigma_\Phi h)^2\ge0$ gives $A\ge-1$. Equality holds exactly when the two profiles are affinely related (Cauchy--Schwarz equality condition).
\end{proof}
...
\begin{remark}[Continuum pathology]\label{sec:pathology}
The continuous integral formulation has an artifact at $L\to0$: $\Sigma\to0$ regardless of how concentrated the integration is, an artifact of the measure of the integration domain vanishing, not a real property of a single-layer system. In practice we use the discrete sum $\Sigma=\sum_\ell w_0(\ell)\Phi(s,\ell,t)$ (similarly for $\Gamma$), with the continuum as the $L\to\infty$ idealization for smooth hierarchies (e.g.\ predictive-processing-style cortical hierarchies).
\end{remark}
...
$\Sigma(\gamma) = \Sigma_+ - \gamma K$, where $K=\sum_{\ell\neq\ell^*}\text{overlap}(\ell,\ell^*)\Phi(\ell)^2$ (note the square: the subtracted term is $w_-(\ell)\Phi(\ell)=\gamma\cdot\text{overlap}\cdot\Phi(\ell)^2$), and $\Sigma_+$ is the competition-free value ($\gamma=0$). The critical threshold is $\gamma_{\text{crit}}=\Sigma_+/K$; for $\gamma>\gamma_{\text{crit}}$, $\Sigma(t)<0$ --- net fragmentation --- becomes reachable.
\end{proposition}
...
\end{tabular}
\end{center}
The competition-mechanism bound $[0.34,0.74]$ correctly contains the observed 0.668. With substrate sets giving Jaccard overlaps $\{0,0.4\}$, $K=0.144$, so $\gamma_{\text{crit}}\approx5.14$; numerically confirmed ($\Sigma(5.14)=0$, $\Sigma(6)=-0.124$).
...
A standard, frozen-weight transformer at inference has $g_\ell(t)\equiv0$ (no online parameter update), hence $\Gamma(t)\equiv0$ by definition, regardless of self-report fluency, and $A(t):=0$ by the boundary convention (no variance in an identically-zero profile).
...
The anesthesia hypothesis above exposes an axis the formalism previously lacked: $(\Sigma,\Gamma,A)$ describes \emph{internal} state, but says nothing about whether that state is legible from behavior. This is precisely the clinical problem that motivated the Perturbational Complexity Index --- a measure explicitly designed to be ``independent of sensory processing and behavior'' \cite{casali2013}.
...
\]
i.e.\ how much knowing layer $\ell$'s state changes the predictability of the effectors' future state, beyond knowing the effectors' current state alone. Structurally identical to $\Phi_{\ell,\ell-1}$ (\S3.1), just directed at a layer$\to$output channel. $R_\ell(t)\ge0$ by the same distance argument, with no further axiom needed. $R_L(t)$ --- the channel from the topmost, self-modeling layer to the effectors --- is of particular interest.

\subsection*{Observed vs.\ actual}
What a clinician observes ($\Sigma_{\text{obs}}$ --- ``visible'' consciousness, from behavior) is bottlenecked by $R_L$, independent of actual $\Sigma$:
\[
\Sigma_{\text{obs}}(t) \le \kappa\cdot R_L(t) \quad \text{for some } \kappa>0.
\]
If $R_L(t)=0$, $\Sigma_{\text{obs}}(t)=0$ \emph{regardless of how high actual $\Sigma(t)$ is} --- a formalization of the risk of conflating $\Sigma_{\text{obs}}$ with $\Sigma$.
...
\]
where $C(t)$ is a contextual signal and $\chi_\ell(t)$ --- a new parameter --- measures how strongly context modulates the error's weight. $\chi_\ell(t)\ge0$ trivially (absolute value), with no upper bound assumed absent further structure on $h$ (as with $R_\ell$).
...
This is a self-consistent conceptual synthesis, not an empirically validated theory. Open items: a systematic (not merely search-driven) literature review; source verification of all citations; resolution of the tension between the two exclusion mechanisms (\S\ref{sec:exclusion}) if the work is developed toward publication; and empirical instantiation on architectures beyond the toy example, particularly to test whether $(\Sigma,\Gamma,A)$ trajectories track known markers of integration (e.g.\ the Perturbational Complexity Index under anesthesia).

\begin{thebibliography}{99}
\bibitem{albantakis2023} Albantakis, L., Barbosa, L., Findlay, G., et al. (2023). Integrated information theory (IIT) 4.0: Formulating the properties of phenomenal existence in physical terms. \emph{PLOS Computational Biology}.
\bibitem{fink2024} Fink, S. B., et al. (2024). Hierarchical consciousness: the Nested Observer Windows model. \emph{Neuroscience of Consciousness}.
\bibitem{usk2026} Author(s) (2026). Consciousness as Uncommon Self-Knowledge: A Synergistic Information Framework. \emph{arXiv preprint}.
...
\bibitem{oswald2023} von Oswald, J., et al. (2023). Transformers learn in-context by gradient descent. \emph{ICML}.
\bibitem{premakumar2026} Premakumar, V., et al. (2026). Unexpected benefits of self-modelling in neural systems. \emph{Philosophical Transactions of the Royal Society A}.
\bibitem{tsm2026} Author(s) (2026). Self-Monitoring Benefits from Structural Integration: Lessons from Metacognition in Continuous-Time Multi-Timescale Agents. \emph{arXiv preprint}.
\bibitem{interoceptive2026} Author(s) (2026). Interoceptive machine framework: Toward interoception-inspired regulatory architectures in artificial intelligence. \emph{arXiv preprint}.
\bibitem{casali2013} Casali, A. G., Gosseries, O., Rosanova, M., et al. (2013). A theoretically based index of consciousness independent of sensory processing and behavior. \emph{Science Translational Medicine}.
...
\bibitem{hotzone2021} Author(s) (2021). How hot is the hot zone? Computational modelling clarifies the role of parietal and frontoparietal connectivity during anaesthetic-induced loss of consciousness. \emph{bioRxiv / NeuroImage}.
\bibitem{sarasso2013} Author(s). Disruption of Frontal--Parietal Communication by Ketamine, Propofol, and Sevoflurane. \emph{Anesthesiology}.
\bibitem{vollenweider2022} Author(s) (2022). A unified model of ketamine's dissociative and psychedelic properties. \emph{Frontiers/PMC}.
...
\bibitem{qela2025} Qela, B., et al. (2025). Predictive-processing deviations across autism, schizophrenia, and depression: a systematic mapping. \emph{Journal TBD}.
\end{thebibliography}
