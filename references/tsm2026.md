...
## Role in Model Σ
The "Temporal Self-Model" (TSM) is a module from this paper that explicitly predicts the
agent's own future hidden states H steps ahead, with a delayed comparison as the
self-prediction loss. In the 2×2 of §14 it is the **only cell satisfying the full definition of
e_ℓ, g_ℓ** (target = internal state × online update) — with the caveat that the update needs to
be confirmed as genuinely online rather than merely trained by standard backprop.
